...
import { runAllTests, TestResult } from './utils/testRunner';
import { MockLLM, LLMResponse } from './utils/mockLLM';

// ============================================
// CLI Database (simplified for standalone use)
// ============================================
...
async function cmdChat(message: string, options: { history?: number; verbose?: boolean }): Promise<void> {
	...
	const llm: LLMClient = isOnline ? realClient : new MockLLM();
	...
	const normalizedMessage = normalizeTranscription(message);
	if (normalizedMessage !== message && verbose) {
		console.log(`\nVoice normalization: "${message}" → "${normalizedMessage}"`);
	}

	// Check for vague request
	if (isVagueRequest(normalizedMessage)) {
		const clarification = getClarificationForVagueRequest(normalizedMessage);
...
function analyzeTone(response: string): ToneAnalysis {
