...
/**
 * Code Analysis Utilities for Proactive Problem-Spotting
 *
 * These utilities help detect common issues in code that TARX can flag
 * proactively, even when the user didn't specifically ask about them.
 */

export interface CodeIssue {
	type: 'security' | 'performance' | 'bug' | 'style' | 'deprecated' | 'error-handling';
 ...
 */
function isLanguageMatch(actual: string, expected: string): boolean {
 ...
 */
export function formatIssuesForResponse(issues: CodeIssue[]): string | null {
 ...
 */
export function analyzeUserCode(message: string): CodeIssue[] {
