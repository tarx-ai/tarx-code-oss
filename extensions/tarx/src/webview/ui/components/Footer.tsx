...
// Chip/CPU SVG icon — ~20px (22% larger than 16px codicons)
...
function getStatusLabel(status: ConnectionStatus): string {
	switch (status) {
		case 'online': return 'LOCAL';
		case 'offline': return 'LOCAL (offline)';
		case 'connecting': return 'LOCAL (connecting...)';
		case 'reconnecting': return 'LOCAL (reconnecting...)';
	}
}

export const Footer: React.FC<FooterProps> = ({ connectionStatus, version, onOpenSettings, onOpenExtensions }) => {
	const [dropdownOpen, setDropdownOpen] = useState(false);
	const [supercomputerEnabled, setSupercomputerEnabled] = useState(true);
	...
	const toggleSupercomputer = (e: React.MouseEvent) => {
		e.stopPropagation();
		setSupercomputerEnabled(!supercomputerEnabled);
	};
	...
	return (
		<div className="tarx-footer">
			{/* Compute Dropdown (opens upward) */}
			{dropdownOpen && (
				<div className="tarx-compute-dropdown" onClick={e => e.stopPropagation()}>
					<div className="tarx-compute-option local">
						<span className="tarx-compute-option-label">
							{getStatusLabel(connectionStatus)}
						</span>
						<span className="tarx-compute-check codicon codicon-pass-filled" />
					</div>
					...
					>
						...
						<div
							className={`tarx-toggle ${supercomputerEnabled ? 'active' : ''}`}
							onClick={toggleSupercomputer}
						>
							<div className="tarx-toggle-knob" />
						</div>
					</div>
				</div>
			)}
			...
			>
				...
				<span className="tarx-footer-label">Compute</span>
				<span className="tarx-compute-chevron codicon codicon-chevron-down" />
			</div>

			{/* Extensions Row */}
			<div className="tarx-footer-row" title="Extensions" onClick={onOpenExtensions}>
				<span className="codicon codicon-extensions" />
				<span className="tarx-footer-label">Extensions</span>
			</div>

			{/* Settings Row */}
			<div className="tarx-footer-row" title="Settings" onClick={onOpenSettings}>
				<span className="codicon codicon-settings-gear" />
				<span className="tarx-footer-label">Settings</span>
			</div>

			{version && (
				<div className="tarx-footer-version">v{version}</div>
			)}
		</div>
	);
};
