...
export class ToggleSidebarVisibilityAction extends Action2 {
	...
	run(accessor: ServicesAccessor): void {
		...
		// TARX: Instead of hiding the sidebar completely, trigger the TARX collapse/expand
		// This calls the tarx.toggleSidebarCollapse command which collapses to 48px icon strip
		commandService.executeCommand('tarx.toggleSidebarCollapse');
	}
}
